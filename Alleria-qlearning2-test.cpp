...
float getDeltaDistance(){
  ...
  distanceNew = 100 - pow(theta1CurrentCount - 4,2);
  cout << "\nBest performance at: " << 4 << endl;
  myfile << "\nBest performance at: " << 4 << endl;
...
const int readDelay = 1;                   //allow time for the agent to execute after it sets its physical state
const float explorationMinutes = 0.1;        //the desired exploration time in minutes 
...
int t = 0;
int main(){
  myfile.open ("static-parabola-performance-4(new-model).txt");
